...
- 컴포넌트 설계 (header, message Bubble, ChatRoom, Unread 등)
- 로그인 상태관리 
- API 연동 (홈피드, 팔로워피드, 프로필, 로그인 및 회원가입)
- 커스텀 훅 및 유틸 함수 작성 (무한 스크롤, 검증 함수)
- 버그 확인 및 수정
...
> 프로젝트 초반에는 화면 구현에만 집중하면서 컴포넌트 간 데이터 전달이 복잡해지고 props drilling 문제가 발생했습니다. 또한 로그인 상태나 사용자 정보처럼 여러 곳에서 공통으로 사용하는 데이터를 개별적으로 관리하다 보니 상태 불일치와 같은 이슈도 겪었습니다. 여기에 더해 작업 계획을 세우지 않고 바로 코드를 작성하는 습관으로 인해 구현 우선순위가 불분명해지고, 코드의 일관성이 떨어지는 문제를 경험했습니다.

> 이러한 시행착오를 통해 리액트는 단순한 UI 라이브러리가 아닌, 상태 변화에 따른 화면 관리와 컴포넌트 재사용성을 극대화하기 위한 구조적인 프레임워크라는 점을 깨달았습니다. 또한 AuthContext를 활용한 전역 상태 관리를 통해 데이터 일관성을 확보할 수 있었고, 개발 전 단계에서 상태 관리 범위와 구조를 미리 설계하는 중요성을 배웠습니다. 마지막으로 작업 단위를 명확히 정의하고 계획을 세운 후 개발하는 습관을 들이면서, 코드의 명확성과 개발 효율성을 모두 높일 수 있었습니다.
